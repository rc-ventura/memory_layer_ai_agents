...
**arXiv:** 2505.08638 (v1 13/05/2025 · v3 23/06/2025, cs.AI) · **Autores:** Darshan Deshpande, Varun Gangal, Hersh Mehta, Jitin Krishnan, Anand Kannappan, Rebecca Qian (Patronus AI) · **Venue:** preprint — o PDF não declara conferência e o metadado do arXiv não traz `journal_ref`; formato ACL (tem *Limitations* e *Ethics Statement*), portanto provável submissão \*ACL, mas **isso é inferência minha, não afirmação do paper** · **Lido:** texto completo + apêndices por subagente (🔎), 2026-09-08 — **promovido a leitura própria (✅) por Rafael em 2026-09-15**, ver [`../../../research-diary/Set/diario_campo_2026-09-14.md#15092026`](../../../research-diary/Set/diario_campo_2026-09-14.md#15092026)

**Dataset:** <https://huggingface.co/datasets/PatronusAI/TRAIL> — licença MIT, mas o repositório está **gated** (`gated: auto`, com cláusula de não-recompartilhamento). Verificado em 2026-09-08: a datasets-server API recusa acesso sem autenticação. Splits (corrigido 16/09/2026, conferido direto na página do dataset — a leitura de 08/09 estava 1 a menos em cada split): `gaia` (**118** exemplos) e `swe_bench` (**30**). Campos: `trace` (string), `labels` (string).
...
### Pergunta em aberto de Rafael (15/09/2026) — dado que a TRAIL estrutura por spans e já usamos Datadog, será que dá pra estruturar de forma parecida?

Rafael não propôs adotar — levantou a pergunta: o TRAIL estrutura o trace como uma árvore de eventos (spans OpenTelemetry, formato JSON/nó), cada passo (chamada de LLM, chamada de tool, resposta do ambiente) isolado com metadados próprios, em vez do `ActionStep` do smolagents que hoje agrega thought+code+observation num blob só — e a [EMPRESA] já usa Datadog. **Será que não dá pra estruturar de forma semelhante?** Isso é genuinamente uma pergunta em aberto, não uma recomendação decidida; a análise abaixo explora se faz sentido, não conclui que se deva fazer. Se fizesse sentido, resolveria na origem a limitação já registrada na linha `location` da tabela acima (nossa unidade de associação de erro é `(exec_id, role, step)`, grosseira; um span TRAIL é uma chamada de LLM **ou** de tool, precisa) — o mesmo tipo de granularidade que `drill_down.py` já tenta dar manualmente, hoje só ao nível de `ActionStep`.

**Ressalva de atribuição, checada contra as datas.** A formulação "o TRAIL adota OTel pra superar uma limitação do AgentDebug" não se sustenta cronologicamente — TRAIL é v1 de 13/05/2025, AgentDebug foi submetido em 29/09/2025, quatro meses depois (confirmado via WebSearch, [arXiv:2505.08638](https://arxiv.org/abs/2505.08638)). Não há relação de sucessão entre os dois; são desenhos independentes e contemporâneos de estruturar trace, não um respondendo ao outro. O correto: **TRAIL estrutura por spans OTel** (unidade = uma chamada de LLM ou de tool, framework-agnóstico); **AgentDebug estrutura por tags XML forçadas no output do próprio modelo** (`<memory>/<reflection>/<plan>/<action>`, depende do agente emitir essas tags — só 2 dos 12 papéis da esteira emitem algo equivalente, o `PlanningStep`, achado 14/09).

**"OpenInference" (Arize) — corrigido 15/09/2026, verificado direto no PDF.** A nota anterior ("não verificado") estava errada — vinha de busca via WebSearch, que não achou a frase; baixando o PDF direto (`arxiv.org/pdf/2505.08638`) e lendo o texto completo, o parágrafo "Workflow Tracing" confirma, verbatim: *"To ensure compatibility of this dataset with real world tracing and observability software, all traces are collected via opentelemetry (OpenTelemetry, 2025), specifically, its most widely adopted open-source derivative compatible with agents, the openinference standard (Arize AI, 2025) as adopted by Moshkovich et al. (2025)."* Confirmado também nos atributos do span de exemplo (Figura 2): `"scope_name": "openinference.instrumentation.smolagents"` e `"openinference.span.kind": "LLM"`. **TRAIL usa OpenInference, explicitamente, citado por nome.**

**Achado novo, verificado 15/09/2026 via WebSearch: compatibilidade real com Datadog.** Rafael observou que [EMPRESA] já usa Datadog. Documentação oficial confirma que o Datadog Agent/LLM (Agent) Observability **ingere nativamente** traces em OpenTelemetry GenAI semantic conventions (v1.37+) **e** em OpenInference, sem precisar do SDK proprietário (`ddtrace`) nem de um Agent local — [OTel Instrumentation docs](https://docs.datadoghq.com/llm_observability/instrumentation/otel_instrumentation/), confirmado também no [blog oficial da Datadog](https://www.datadoghq.com/blog/llm-otel-semantic-convention/). Frameworks que já emitem spans nesse padrão (ex.: Strands Agents da AWS) são ingeridos automaticamente. **Não confirmado:** se existe instrumentação automática equivalente pro `smolagents` (framework da esteira) — precisaria checar a lib de instrumentação OTel do Datagod/OTel ou instrumentar manualmente via SDK OTel dentro do framework.

**Checado no trace cru (15/09/2026, via `drill_down.py`/`pandas` direto no `.csv.xz`, não por memória) — a resposta não é um "sem span" plano.** Rodei uma verificação estrutural nos 840 registros/8.396 steps: nenhuma chave span/OTel padrão (`span_id`, `trace_id`, `parent_span_id`, `dd_trace_id`, `otel_*`, `attributes`, `resource`) existe em nenhum step — isso confirma que **não há spans no sentido OTel** (sem hierarquia trace→span, sem contexto de propagação entre serviços). Mas o `ActionStep` **já carrega matéria-prima de span**, só não exposta como tal: um campo `timing` com `{start_time, end_time, duration}` por step (timestamp Unix real, não estimado), e `tool_calls`/`model_output_message.tool_calls` com um `id` por chamada de ferramenta dentro do mesmo step (formato estilo OpenAI: `id`/`type`/`function`). Ou seja: **não são spans, mas os ingredientes de um span (timestamp de início/fim, id por sub-chamada) já estão sendo coletados** — reestruturar isso em spans OTel de verdade (um span por chamada de LLM, um por chamada de tool, ligados por `trace_id`/`parent_span_id`) é mais uma transformação de forma sobre dado já existente do que instrumentação do zero.

**Se a resposta for sim, onde isso se aplicaria — instrumentação futura da plataforma, não reanálise retroativa do dataset atual.** O CSV já coletado é serialização nativa do smolagents (`TaskStep`/`ActionStep`/`PlanningStep`, achado 14/09, roadmap item 10) — sem `span_id`/`trace_id`, então não dá pra "minerar spans" que não existem como tal neste arquivo; a pergunta vale pra frente, pro que a plataforma loga daqui em diante (Sub 2.5, pontos de integração), aproveitando que `timing`/`tool_calls.id` já são coletados hoje. O que a pergunta ganha de plausibilidade, e não tinha antes de checar: com Datadog já em uso, o caminho de adoção é mais barato do que parecia — reaproveita infra existente em vez de subir uma stack de observabilidade nova, mesmo princípio já usado pra decidir reuso do Signal Ledger (`../../../discussion/open-questions.md`). Se resolvida, essa granularidade de span endereçaria a "Camada 2" de causa ainda não alcançada (`04-roadmap.md`, nota "Camadas de causa") — separar, sem regex/AST, qual sub-parte de um `ActionStep` carrega o erro.

**Sobre "tag plan" — esclarecido, sem contradição.** Rodei de novo agora, direto no trace (não reaproveitando o achado de 14/09 de memória): busquei `<think>`, `<thinking>`, `<reasoning>`, `<plan>`, `<memory>`, `<reflection>`, `<action>`, `<observation>` como **tag XML literal** em `model_output`, `code_action`, `observations` e `model_input_messages`, nos 8.396 steps, todos os papéis — zero ocorrências, confirmando de novo que nenhum papel emite tag XML de raciocínio como texto (`04-roadmap.md`, achado 14/09). Rafael esclareceu (15/09) que a referência não era a uma tag XML literal, e sim ao **campo `plan` do `PlanningStep`** — o mesmo achado de 14/09: só 2 dos 12 papéis o emitem, `RespostaBacen` e `CalculoTrabalhista` (10 e 2 `PlanningStep` respectivamente, censo em 200 execuções, `04-roadmap.md` item 10). É um campo estruturado isolado pelo framework (smolagents), não uma tag XML dentro de um blob de texto — mais parecido com o "location = span id" do TRAIL (um pedaço de dado já separado por metadado) do que com o `<plan>` do AgentDebug (que é literalmente parte do texto livre que o modelo escreve). As duas checagens acima — "zero tags XML literais" e "PlanningStep.plan existe em 2 papéis" — são compatíveis, não contraditórias.
...
> **Nota (15/09/2026): por que `Tool Selection Errors` não aparece nesta tabela.** Esta tabela é a distribuição **só do split SWE-Bench** (`swe_bench=0` pra Tool Selection Errors — as 45 ocorrências do dataset são todas de GAIA, ver "A taxonomia completa" acima). Isso não o exclui da nossa lista de detectores: ele é o **item #2** da tabela de análises executáveis (§4 abaixo), incluído por custo×precisão (exemplo canônico da Figura 2 do paper), não por frequência observada nesta arquitetura comparável. Ver "Por que estas 11..." abaixo pra lógica completa de seleção.
...
| **12** | **Tool Output Misinterpretation** (2,0%) — **adicionada 15/09/2026, documentada e não descartada** | `code_action` (o que a ferramenta retornou) × `model_output` do step seguinte (o que o agente assumiu que o retorno significava) | Sem fingerprint sintático conhecido hoje — exige julgar se o agente **entendeu corretamente** o retorno antes de usá-lo, não só se usou um nome errado (isso é o item #2) ou afirmou algo nunca chamado (isso é o item #7). Faz sentido continuar de olho nela; só não tem proxy determinístico barato ainda | **Alto** | Mesma classe de #9/#10 — análise semântica, provável LLM-as-judge. Casa quase palavra por palavra com `Outcome Misinterpretation` do módulo `reflection` do AgentDebug, que o `04-roadmap.md` (item 10) já tinha marcado como "fora de alcance sem LLM" nesta base |

**Sugestão de sequenciamento:** #1, #2, #7 numa rodada (todos são AST/hash, um dia de trabalho, três categorias TRAIL novas); #3 na rodada seguinte, isolada, porque é o achado de maior valor de tese; #4 e #5 depois; #9/#10/#12 só quando houver um pipeline de LLM-as-judge validado — os três ficam juntos na mesma fila de espera, não descartados.

### Por que estas 11, e não as outras ~10 categorias-folha — lógica de seleção reconstruída (15/09/2026)

Isso não estava escrito em lugar nenhum até agora — só dava pra reconstruir cruzando esta tabela com a tabela completa das 20 categorias-folha (§"A taxonomia completa" acima) e a distribuição por área (§1 acima). Reconstruído a pedido do Rafael, depois de uma explicação minha anterior (que usava só a tabela do split SWE-Bench, linhas 214–230) não se sustentar contra o item #2.

**A regra real não é frequência num split — é "descreve algo que pode existir na nossa esteira e que `classify()` (baseado em exceção) não vê hoje":**

- **Todas as 8 categorias de `Reasoning Errors`, exceto `Tool Output Misinterpretation`** → 7 entram (#2, #3, #4, #7, #8, #9, #11).
- **Todas as 4 categorias de `Planning and Coordination Errors`** (Context Handling Failures, Resource Abuse, Goal Deviation, Task Orchestration) → todas entram (#1, #5, #6, #10).
- 7 + 4 = **11**.

**O que fica fora, e por quê:**

- **7 das 8 categorias de `System Execution Errors`** (Environment Setup, Rate Limiting, Authentication, Service Errors, Resource Not Found, Resource Exhaustion, Timeout Issues) — fora porque **não são ponto cego pra nós**: a comparação por área (§1 acima) mostra que já vemos **5,2%** dessa família via exceção — proporcionalmente **mais** que o próprio TRAIL mede (3,7%) — e todas essas 7 têm assinatura de exceção plausível no trace (`classify()` já pega `Error code: 422`/HTTP, por exemplo). Não tem gap pra fechar; construir detector aqui seria esforço sem retorno.
- **`Tool Definition Issues` — argumento diferente, checado 15/09/2026 (o argumento de paridade de área acima não cobre este caso individualmente).** Duplamente fraco: (a) **frequência** — `swe_bench=0, gaia=3, total=3, 0,4% de 841`, uma das duas categorias mais raras de toda a taxonomia; escalado pro nosso volume (498 erros) seria ~1,8 caso esperado, sem massa pra justificar o esforço; (b) **tipo estrutural, não só raridade** — a definição do TRAIL (*"the tool was not defined correctly by the user or contains some errors that make it inconsistent with its description"*) é um defeito de **autoria da ferramenta** (schema/docstring declarado errado), não algo que acontece **durante** uma execução — não é mineração de trace (`model_output`/`code_action`/`observations`), é auditoria do código/documentação de cada tool contra seu comportamento real, uma vez, fora do escopo desta tabela ("análises executáveis **sobre os dados que já temos**").
- **`Domain Specific Errors`** — fora porque o TRAIL não tem um único exemplo anotado dessa categoria em todo o dataset (não aparece na Figura 3a).
- **`Tool Selection Errors` é a exceção que entra apesar de n=0 no split SWE-Bench** (`swe_bench=0, gaia=45` — ver tabela completa acima) — a inclusão não é por frequência observada na arquitetura comparável, é por **custo×precisão**: a Figura 2 do TRAIL dá um exemplo canônico do mecanismo exato (thought anuncia uma ferramenta, código chama outra), detectável por `ast.parse` + regex, custo baixo, zero LLM, plausível de ocorrer na nossa arquitetura mesmo sem termos visto ainda.

**`Tool Output Misinterpretation` — corrigido 15/09/2026: não é descartada, é documentada como item #12, rank Alto custo.** Na primeira versão desta nota eu tinha argumentado só "por que ficou fora" — Rafael corrigiu o enquadramento: faz sentido continuar de olho nela, ela só está no rank de custo mais alto (junto com #9/#10) porque exige análise semântica, não porque foi descartada. Já virou linha própria na tabela acima (#12). Definição verbatim: *"Made assumptions about the tool output or used the tool output in an incorrect context."* Ao contrário de `Tool Selection Errors` (assinatura estrutural limpa — nome anunciado ≠ nome chamado, checável por AST) ou `Tool-related Hallucination` (capacidade afirmada nunca invocada, também AST), esta não tem fingerprint sintático conhecido hoje: exigiria julgar se o agente **entendeu corretamente o significado** de um retorno de ferramenta antes de usá-lo.

**Reforço independente, cruzando com o outro paper já lido nesta sessão** (isto é o que sustenta o rank Alto, não uma exclusão): a definição do TRAIL casa quase palavra por palavra com **`Outcome Misinterpretation`** do módulo `reflection` do AgentDebug (fichamento `agentdebug-2509.25370.md`: *"Executes an action but misreads the immediate result or environment feedback."*) — mesmo fenômeno, dois papers, dois nomes. E o próprio `04-roadmap.md` (item 10, "2º sub-achado", 14/09) já tinha concluído, **antes e independentemente desta análise**, que os módulos `memory`/`reflection` do AgentDebug estão "fora de alcance sem LLM" nesta base — `model_output` é um blob só, sem como separar mecanicamente "lembrou errado" de "interpretou mal o resultado anterior" sem rodar um Detector Prompt (LLM). Duas fontes independentes apontando pro mesmo limite é evidência razoável de que o custo é real, não descuido.

**Isto é reconstrução/argumento meu (15/09/2026).** Se alguém encontrar um proxy determinístico barato (ex.: comparar o tipo/schema do retorno da ferramenta contra o tipo que o *thought* seguinte assume, de forma análoga ao item #11), o item #12 desce de rank — não é parede definitiva, é "não achamos um proxy ainda", igual a #9/#10.
...
3. **Dados estruturados são difíceis para LLM** (citam Guo et al. 2023; Sui et al. 2024) — citação verbatim e gloss corretos na Ressalva 3 abaixo.
...
3. **Citação verbatim da motivação/related work — o que ela sustenta e o que não sustenta (checado 15/09/2026, a partir do trecho que Rafael colou do PDF).**

   > *"Previous agent trace analysis frameworks have primarily focused on parsed traces containing unstructured text (Cemri et al., 2025), which do not adequately represent common agent framework outputs that generate structured traces logged in standardized formats like opentelemetry (OpenTelemetry, 2025). Additionally, as observed by Guo et al. (2023); Sui et al. (2024), handling structured data remains challenging for LLMs, an observation corroborated by previous research on automated software engineering trace analysis."*

   São **duas afirmações distintas**, e nenhuma delas é sobre o agente em si:
   - **Fidelidade do dataset de análise, não do agente:** a crítica ao MAST/Cemri et al. é que ele *parseia* o trace em texto solto para analisar, o que não representa fielmente o formato nativo (structured/OTel) que frameworks de agente de fato produzem. É uma crítica ao **método de análise** de trabalhos anteriores, não uma afirmação sobre o que "contamina" o raciocínio do agente durante a execução — o agente não é mencionado nesta frase.
   - **Dificuldade é do LLM, não do anotador humano.** "Handling structured data remains challenging for LLMs" é especificamente sobre modelos, citando Guo et al. 2023 e Sui et al. 2024 (análise automatizada de trace de engenharia de software). Não é uma afirmação sobre anotadores humanos — e o próprio TRAIL contradiz a leitura de que estrutura dificulta anotação: seus 4 anotadores trabalharam span a span (*"evaluated each LLM and tool span in sequence, marking span ID..."*, já citado acima) com ~5% de taxa de revisão em 4 rodadas — estrutura deu ao anotador uma unidade precisa pra julgar, não um obstáculo.

   **O que isso sustenta pra nós, então:** não "trace bruto contamina o contexto do agente" (não é isso que o trecho diz) nem "estrutura é difícil pra anotador" (o trecho diz o oposto, e a metodologia do TRAIL o confirma) — e sim que (a) span/unidade fina é o formato mais fiel pra *análise offline* e ajuda o anotador humano a ser preciso (razão pela qual a pergunta sobre estruturar nosso próprio log em spans, registrada em `open-questions.md`, faz sentido do lado da anotação), e (b) "dados estruturados são difíceis pra LLM" é a razão nº3, secundária, do juiz LLM ir mal no benchmark — a razão nº1, medida (não só citada), é comprimento de entrada (Ressalva/veredito acima). As duas leituras são compatíveis com o veredito já registrado: **span ajuda quem anota; step pequeno (não trace inteiro) é o que ajuda o LLM.**
...
7. **Inconsistências internas menores, para não sermos pegos.** (a) **Corrigido 16/09/2026** (a leitura de 08/09 tinha os números do HuggingFace errados, 1 a menos em cada split — ver "Dataset" acima): o texto diz 148 traces; a Tabela 5 lista 118 GAIA + 31 SWE = 149; o HuggingFace publica **118 GAIA + 30 SWE-Bench = 148**. A divergência real é mais estreita do que a leitura anterior sugeria: GAIA bate exato entre Tabela 5 e HuggingFace (118 = 118); só o SWE-Bench diverge (31 na Tabela 5 contra 30 no HuggingFace) — um único trace de diferença, não dois. (b) *Rate Limiting* está na taxonomia e aparece como exemplo anotado na Figura 2, mas não tem barra na Figura 3a. (c) O gráfico exibe rótulos singular/plural duplicados (`Formatting Error` n=1, `Context Handling Failure` n=5), sugerindo normalização imperfeita dos rótulos de anotação. Citar contagens do nível de tipo com essa margem.
