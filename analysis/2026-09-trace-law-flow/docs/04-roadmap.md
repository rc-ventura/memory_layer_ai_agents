...
| 27 | **Confirmar com a esteira o que é `anomesdia`** — hipótese: data de corte da democratização da base (1 valor por mês, sempre posterior à execução). Perguntar também como um recorte "de um mês" é feito: por lote ou por data de execução — decide se a base 2 é de fato 1 mês | `05-schema.md` §Datas e §Aberto 9 · `03-procedimento-validacao.md` §1.12 |
