...
| 21 | **Reenquadrar o §8.8 como "papel → taxonomia de erros" (descritivo puro)** — **parte feita em 24/09:** o §8.8 só descreve (papel × unidade, cada unidade com sua cor, sem "candidata/não vira memória"); a decisão fica no §9 e na genealogia, que passou a ser pintada pela decisão (`03-procedimento-validacao.md` §1.14). **Continua aberto: a terminologia "unidade/candidata" precisa ser revista** — o que confunde é chamar de "unidade de memória" um agrupamento que inclui erros que nunca viram memória. Extensão possível: hoje `triagem()` decide por unidade **global** (conta execs/meses da unidade inteira); candidatura por (papel × unidade) seria análise nova | `analise_trace_esteira_juridica.ipynb` §8.8/§9.3 · `base_pipeline.py` `triagem` |
