...
| 13 | **5 detectores ToolScan/ToolFailBench já desenhados no fichamento, nunca construídos nem virados item de roadmap** | Achado 16/09/2026, revisando `../literature/tool-use-errors.md` §"Tradução para o paradigma CodeAgent" (tabela da linha 183) a pedido do Rafael. A tabela já traduz os 7 tipos do ToolScan (IAC/IAV/IAN/IAT/RAC/IFN/IFE) + os 4 modos do ToolFailBench (Tool-Skip/Result-Ignore/Output-Fabrication/Unnecessary-Tool-Use) pro paradigma CodeAgent, com detector concreto (AST/regex) e uma ordem de execução por valor÷esforço própria — e isso nunca virou item deste roadmap; só o TRAIL (item 12 acima) tinha tabela equivalente promovida a item de ação. Do que a tabela já desenha, faltam 5: **Output-Fabrication** (ToolFailBench, custo D+J) — **é literalmente o mesmo mecanismo do item 1 deste roadmap** ("Groundedness-como-presença": extrair token tipado do `final_answer` — CNJ, CPF/CNPJ, data, valor — e checar suporte nas observações anteriores); os dois vinham sendo tratados como achados separados, a conexão nunca tinha sido escrita antes de hoje. **IAT** — tipo de argumento errado (ToolScan, custo D+, precisa das assinaturas reais das tools — inferência de tipo sobre literais vs. anotação declarada). **IAC** — chamadas insuficientes (ToolScan, custo **J**, sem proxy determinístico bom — o próprio paper chama de *"the most common error pattern"* nos modelos que testou; o fichamento aposta que é "provavelmente seu maior balde silencioso" aqui também, mas isso é hipótese, não medido). **Unnecessary-Tool-Use** (ToolFailBench, custo D) — análogo fraco nesta esteira orientada a tarefa (não é Q&A aberto), mas tem variante barata que reaproveita o Result-Ignore: chamada a subagente caro (`ConversationAgent`, `answer_question_using_documents`) cujo resultado é descartado. **Subclassificação do IFE** (ToolScan, custo D) — os 223 `SyntaxError` já contados em bloco único, quebrados por causa sintática via `ast.parse`/`SyntaxError.msg` (cerca sem fechar, aspas triplas, f-string, prosa misturada a código); cada subclasse vira candidata a unidade de memória procedural própria, em vez de ficar amassada numa família só. **Fora da lista, por já terem sido testados e não compensarem virar detector permanente:** IAN/IAV do ToolScan (1 caso de IAN, zero de IAV, achado no exercício de quebrar o balde `TypeError` — `02-relatorio-achados.md` §7 item 1); Wrong-Answer do ToolFailBench (exige gabarito jurídico, fora de alcance). **Ordem sugerida pelo próprio fichamento** (`tool-use-errors.md` §"Ordem de execução recomendada", por valor÷esforço): Output-Fabrication primeiro — maior risco no domínio jurídico e não depende de nenhum dado novo, roda na base já existente hoje; IAT e Unnecessary-Tool-Use depois; IAC por último (mais caro, único que pede juiz sem triagem determinística antes). **Pendência lateral, não bloqueante:** ToolScan e ToolFailBench seguem em leitura 🔎 (por subagente, `literature/README.md`) — nenhum promovido a ✅ (leitura própria do Rafael) como já foi feito com TRAIL (15/09) e AgentDebug (09/09); promoção é prioridade do Rafael, pode acontecer em paralelo à construção destes detectores, não precisa vir antes. |
| 14 | **Curva de erro por maturidade da ferramenta — detectar erro por "contato com ferramenta nova"** | Pergunta de 17/09/2026: a taxonomia atual (`classify()`, os 90 nomes declarados) generaliza se o harness ganhar ferramentas novas, ou é presa a este toolset? Hoje **não dá pra medir** especificamente "erro por a ferramenta ser nova" — o inventário de ferramentas é extraído do system prompt por execução (fonte autoritativa, `01-racionais.md` §2), mas depois é colapsado numa **lista única fixa** somando as 1.000 execuções/9 meses; qualquer diferença de "quando a ferramenta apareceu" se perde nesse colapso (mesma causa-raiz do item 5 nunca ter convergido). Proposta: (1) manter o inventário **por execução**, não só a união; (2) achar a **primeira execução em que cada ferramenta aparece declarada** (usando `anomesdia`) — isso marca o "nascimento" dela no trace; (3) cruzar com `cod_vers_aget` (coluna de versão do agente, `docs/schema.md` linha 29 — existe no schema e nunca foi usada em nenhuma análise); (4) para cada ferramenta, comparar a taxa de erro nas primeiras N execuções após o nascimento contra a taxa de erro dela mais adiante — se cair com o tempo de exposição, é evidência real de efeito de novidade; se não cair, o "erro" não é sobre a ferramenta ser nova. Mesmo princípio de teste de robustez já usado no resto do documento (variar o corte, ver se o achado se sustenta), aplicado ao eixo tempo-de-vida-da-ferramenta em vez de corte de amostra ou definição de mensagem igual. **Motivação:** é o teste mais direto disponível para o blind spot já registrado em [`../../../discussion/open-questions.md`](../../../discussion/open-questions.md) (linha 128) — uma falha silenciosa nova (sem exceção Python, sem bater em nenhum detector já catalogado) hoje só aparece por inspeção manual de execução via `drill_down.py`; uma ferramenta nova com taxa de erro anormalmente alta no início seria flagrada por este teste antes de alguém precisar ler caso a caso. **Bloqueado em:** confirmar com o time da esteira se `cod_vers_aget` de fato corresponde a mudança de toolset — pergunta já na lista de "Aberto" do schema (`docs/schema.md` linhas 138–148), ainda sem resposta. **Não cobre:** mudança de *convenção* de declaração de tools (ex.: harness migrar de assinatura textual no system prompt para tool-calling estruturado) — isso quebra o parser do item, não é ajuste de régua, é reescrita do extrator; fica registrado como limite do método, não como próximo passo deste item. |
