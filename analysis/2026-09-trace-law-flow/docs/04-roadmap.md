...
| 31 | **Timeout de ferramenta (base 2).** 16 erros, 15 execuções, 1 mês, 1 papel; a mensagem é em português (wrapper da esteira, não do smolagents) e o agente segue com o fallback do prompt. Reconhecer no `classify()` (sintoma novo, família de plataforma) e abrir `H_*` não-memória com dono e gatilho de reabertura — antes, ver se os 16 são incidente (poucos dias) ou estrutural (timeout curto demais). A falha silenciosa por trás (resposta final sem a etapa do laudo) vai para o item 26 | `../../pipeline-entre-bases.md` §5 · `03-procedimento-validacao.md` Frente 3 (não-memória) |
