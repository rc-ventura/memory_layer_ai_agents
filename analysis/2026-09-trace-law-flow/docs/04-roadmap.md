...
**Atualizado:** 2026-09-25 (resíduo da base 2: Ajustes 2.1 e 2.2, itens 29–36; antes, 23/09: datação por `dat_hor_inio_exeo`, auditorias a refazer, errata, item 27). Em 19/09 passou ao formato simples — check + frase +
...
| 21 | **Reenquadrar o §8.8 como "papel → taxonomia de erros" (descritivo puro)** — **parte feita em 24/09:** o §8.8 só descreve — um Pareto de erros por papel (submecanismo: os 3 maiores do agente + plataforma + resíduo + outros), sem "candidata/não vira memória"; a decisão fica no §9 e na genealogia (`03-procedimento-validacao.md` §1.14). **Continua aberto: a terminologia "unidade/candidata" precisa ser revista** — o que confunde é chamar de "unidade de memória" um agrupamento que inclui erros que nunca viram memória. **Extensão feita em 24/09: candidatura por (papel × unidade)** — `triagem_por_papel()` roda a mesma régua (≥3 execs, ≥2 meses) dentro de cada papel; §9.4 = matriz papel × unidade (18 candidatas de 33 células), §9.5 = cruzamento global × papel (14 herdadas, 0 reveladas — impossível por construção com a mesma régua; 8 de 18 limítrofes na régua estrita). Racional: `01-racionais.md` §7 Passo 10; validação: `03-procedimento-validacao.md` §1.15 | `analise_trace_esteira_juridica.ipynb` §8.8/§9.3–9.5 · `base_pipeline.py` `triagem`/`triagem_por_papel` |
...
| 29 | **Resíduo da base 1: o padrão dos parênteses — refeito em 25/09 com a base 2 rodada.** `SyntaxError: closing parenthesis <q> does not match opening parenthesis <q>` segue em "revisar — prioridade" na base 1 (3 execuções, 2 meses), mas no cru **não é um mecanismo só**: 2 são erros de digitação reais e 1 é texto longo dentro de uma chamada; na base 2 o mesmo motivo aparece em 7 erros do `RoteadorCivel` que são **retorno colado** (`repr_colado`, Ajuste 2.2). Falta decidir: regra por tipo de parser para a digitação, ou manter como cobertura | `03-procedimento-validacao.md` §1.16 · `../../pipeline-entre-bases.md` |
| 30 | **`?` — a chave que não identifica (base 2).** 7 erros: 6 `AgentExecutionError` sem `due to` (5 execuções, 3 meses, 3 papéis) + 1 `AgentMaxStepsError`. Ler os 6 no cru (uma categoria por caso); `AgentMaxStepsError` é plataforma; e mudar `padrao_residuo()` para que uma chave sem impressão digital (`?`, ou só a classe) não conte como recorrência, usando o `error.type` | `../../pipeline-entre-bases.md` §5 · `base_pipeline.py:padrao_residuo` |
| 31 | ~~**Timeout de ferramenta (base 2)**~~ — **feito em 25/09 (Ajuste 4)**: família `Infra / ferramenta`, unidade `H_timeout_ferramenta` (não-memória), gatilho em § Monitoramento; base 1 sem mudança. Falta replicar e conferir na base 2. A falha silenciosa por trás (resposta final sem a etapa da ferramenta; 16/16 terminam com `final_answer`) segue para o item 26 | `../../pipeline-entre-bases.md` §3 Ajuste 4 |
| 32 | ~~**`Import from X is not allowed` (base 2)**~~ — **feito em 25/09 (Ajuste 3)**: a regra do sandbox casa `Import of` e `Import from`; base 1 sem mudança. Conferido na base 2 (causa não identificada 12 → 7) | `../../pipeline-entre-bases.md` §3 Ajuste 3 |
| 33 | **Cruzar padrões do resíduo entre bases conferindo o mecanismo.** O `final_answer` com argumento inexistente (nas duas bases, com nomes de classe diferentes) já ganhou regra no Ajuste 3; falta o caso geral — a chave dos parênteses (item 29) junta mecanismos diferentes e a máscara separa mensagens iguais. Definir a comparação por classe de exceção + mecanismo, não pela frase mascarada | `03-procedimento-validacao.md` §1.16 · Frente 3 |
| 34 | **Robustez do alarme de cobertura.** Na base 2, 5,2% de sintoma não reconhecido eram 64% um único padrão (Timeout, 16 de 25) num mês; sem ele, 1,9%. Mostrar na saída a concentração (maior padrão, taxa sem ele, meses), contar o alarme sobre ocorrências (a triagem usa ocorrências, o alarme conta erros) e decidir se o critério de 2 meses vale para plataforma | `base_pipeline.py:triagem` · `../../pipeline-entre-bases.md` §5 |
| 35 | **Achados laterais do caso `repr_colado` (ConversationAgent, mar/2026, base 1).** (a) O step seguinte falha em "Could not index" com causa de raiz de contrato dict (`docs_summary[0]` num dict `{'result': …}` dá lista vazia): conferir se o `submecanismo()` manda esses casos à `U_contrato_dict`. (b) A resposta final diz "Nenhum documento encontrado" com a ferramenta tendo devolvido documentos — falha silenciosa, para o item 26 | `03-procedimento-validacao.md` §1.16 |
| 36 | **Auditoria nº 6, checagem E fora de fase.** Usa o mês do lote (`202512`) e espera 26/33 erros de "Explicação solta" em dez/2025; com o relógio da execução são 25/33 em out/2025 (`01-racionais.md` §7 Passo 7). Atualizar a expectativa e o texto; não afeta as outras checagens (A, B, C, G dão 0 divergências) | `audit/scripts/audit_recompute6.py` |
| 37 | **A nº10 aparece como memória nas figuras, mas a mineração decidiu `harness`.** "Campo inexistente no retorno estruturado" é erro do agente e a triagem automática a faz candidata (MEM no Sankey, seção de candidatas no 9.3); a mineração (07 §6.1) decidiu `validation.destino = harness` (9/21 respostas com payload inválido). Decidir se as figuras mostram a decisão pós-mineração ou só a triagem automática (e dizer isso na legenda) | `07-relatorio-mineracao-unidades-n2-n10.md` §6.1 · `09-metodologia-erro-a-memoria.md` §1.1 |

## Monitoramento — gatilho de reabertura

- [ ] **"Ferramenta excedeu o timeout" (`H_timeout_ferramenta`, Infra / ferramenta) — gatilho ACIONADO na base 2.**
  Regra: ≥2 casos num mês ou mais de 1 a cada 1.000 steps (a mesma do Protocolo do harness). Base 2: 16 erros em
  ago/2026, só `ContestacaoCivel`, 5 ferramentas, limites de 600/1800 s, 12 em 10–11/08 e 4 até 25/08; base 1: 0.
  **Dono:** plataforma da esteira (fluxo `ContestacaoCivel`). **Ação:** ticket com as ferramentas, os limites, o pico e a
  cauda — incidente (pico) ou limite curto demais (cauda). → `../../pipeline-entre-bases.md` §3 Ajuste 4
- [x] **"Resposta sem bloco de código" (Protocolo do harness)** — inativo só *dentro da base 1* (1.000
  registros, out/2025–ago/2026): não assumir resolvido nem inativo, como este item já avisava. **Gatilho
