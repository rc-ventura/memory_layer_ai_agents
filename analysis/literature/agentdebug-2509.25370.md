...
7. **Arquitetura modular imposta por prompt — matizado 14/09/2026.** O método pressupõe que o agente emita `<memory>/<reflection>/<plan>/<action>` separadamente em tags XML. A minha esteira não emite essas tags — mas **não é verdade que tudo funde em `model_output`**: censo em 200 execuções mostrou que 2 dos 12 papéis (`RespostaBacen`, `CalculoTrabalhista`) emitem um `PlanningStep` nativo do smolagents, com campo `plan` isolado, cujo texto bate com o template próprio do framework (*"Here are the facts I know and the plan of action..."*) — não é a tag do paper, mas é atribuição **real** de módulo `planning` pra esses 2 papéis, não analogia. Pros outros 10 papéis, a fusão reflection+planning em `model_output` segue valendo. Achado completo e censo em `analysis/2026-09-trace-law-flow/docs/04-roadmap.md` item 10.
