...
**GraphRAG — ativado 20/08/2026, confirmado 24/08/2026 por inspeção direta do código.** Não foi o fluxo jurídico que decidiu (essa pergunta segue em aberto — ver [`../discussion/open-questions.md`](../discussion/open-questions.md)). A transcrição da reunião de infra (21/08) descreveu verbalmente algo que soava mais como checkpoints de estado do LangGraph (conversa → checkpoints → resumos incrementais) do que GraphRAG propriamente — isso motivou uma correção temporária rebaixando o motivo da leitura. Rafael então checou o código do Fed diretamente e confirmou que o componente GraphRAG existe de fato — provavelmente convivendo com o LangGraph (LangGraph como orquestração/estado da conversa, GraphRAG como camada de conteúdo/retrieval por cima). Ver [`../discussion/checkpoint-2026-08-21-infra-reflections.md#3`](../discussion/checkpoint-2026-08-21-infra-reflections.md#3-langgraph-checkpointing-and-graphrag--both-not-eitheror). Motivo de leitura volta a ser forte: avaliar e potencialmente integrar com o que já existe, não só panorama teórico. Verificados via busca (título/autores/data confirmados, não lidos ainda):
