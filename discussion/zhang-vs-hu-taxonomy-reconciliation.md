...
## A candidate fourth anchor (25/08/2026)

MemEvolve (arXiv:2512.18746, ICML 2026 — see [`../papers/reading-queue.md`](../papers/reading-queue.md#surfaced-for-the-cross-survey-taxonomy-synthesis-pendency-25082026)) proposes its own modular design space — `(E, U, R, G)`: encode / store / retrieve / manage — empirically validated by reimplementing twelve existing memory systems in a common codebase, rather than derived from reading surveys the way Zhang's and Hu's axes were. Worth checking, when the pending three-survey synthesis actually happens, whether `(E,U,R,G)` reconciles cleanly with Zhang's Operations (Writing/Management/Reading) and Hu's Dynamics (Formation/Evolution/Retrieval) the same way those two reconciled with each other above — or whether it cuts the lifecycle differently enough to need its own bridging paragraph.
