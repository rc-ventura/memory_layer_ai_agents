...
**Primary source for this decision:** the [20/08 tutor checkpoint](../docs/checkpoints/checkpoint-2026-08-20-tutor-kickoff.md) ([transcript](../docs/sources/Checkpoint_Tutor_2026-08-20.docx)) is where this was actually settled, not just recorded. The tutor described the desired mechanism unprompted ("corrija a memória... para tentar diminuir ao longo do tempo esses casos errados"); Rafael checked it against the RL framing ("só reforçando, não retreinando... mexendo no harness... uma skill nova, uma memória de procedure"); the tutor confirmed ("Perfeito, exatamente isso"). See [`checkpoint-2026-08-20-reflections.md#1`](reflexoes/checkpoint-2026-08-20-reflections.md#1-the-tutor-independently-confirms-the-harness-not-retraining-decision--with-the-exact-vocabulary) for the full exchange and what else that meeting confirms (the cross-agent scope narrowing in decision #3 below also gets real-world corroboration there).

**Third reinforcement, 28/08/2026 ([tutor checkpoint](../docs/checkpoints/checkpoint-2026-08-28-tutor.md), reflections [`checkpoint-2026-08-28-reflections.md#6`](reflexoes/checkpoint-2026-08-28-reflections.md)).** Asked directly, the tutor reconfirmed retraining as out of scope and added a rationale the log didn't have: he considered fine-tuning small per-agent models and rejected it on **inference opex** — keeping GPUs running costs more than API calls, even quantized and with frozen weights — plus "1 year with two people is too little to build a real RL pipeline; that needs a dedicated team." His exact framing: *"descartado totalmente? Acho que não"* — deprioritized with a clear cost argument, not closed forever, and *"guiar o modelo com o texto é muito mais barato, e os modelos estão cada vez mais obedientes."* Not a new decision — a citable cost-side reinforcement of the existing one.
...
**Partial resolution (24/08/2026, agent-read via sub-agents, not Rafael-read):** a full-text reading sprint ([`reading-sprint-2026-08-24-queue-papers.md`](reading-sprint-2026-08-24-queue-papers.md)) confirms **Mem-α** (arXiv:2509.25911) belongs in the same bucket as Retroformer/Memory-R1, not the PE family — it fine-tunes the agent's own backbone LLM (Qwen3-4B) via GRPO, with a measured base→post-RL jump (0.389→0.642) confirming real gradient updates to the acting model. **Memento** (arXiv:2508.16153) is a third, distinct case: its LLM stays frozen (non-parametric w.r.t. the agent itself), but its best-performing read variant trains a *separate* neural Q-network controller via real RL gradients — not prompt engineering either, just a smaller and different target for the gradients. Net result: **none of Mem-α, Memento, or Retroformer turn out to be PE-based**, and the non-parametric candidates in the same sprint (ExpeL, SCM) aren't really "RL" in a formal sense either — they're heuristic self-curation (vote/prune, LLM-judgment gating) without a reward-optimized policy. This suggests the reading corpus may not contain a clean RL-via-PE precedent at all — the audit isn't fully closed (Synapse/MetaGPT/TiM/RecAgent/S³ and Casper et al. remain unread), but the shape of the answer so far is "this family may need to be defined by the project itself, not adopted from the corpus."

**Candidate precedent surfaced 27/08/2026 (bibliographic verification only — WebSearch confirmed title/venue/mechanism; not read by anyone):** **REMEMBERER / RLEM** (*Reinforcement Learning with Experience Memory*, "Large Language Models Are Semi-Parametric Reinforcement Learning Agents", arXiv:2306.07929, NeurIPS 2023) is the closest thing in the corpus to a genuine RL-that-updates-memory-not-weights mechanism. The LLM stays frozen; the "policy" is a **table of `(observation, action, Q-value)` tuples** whose Q-values are updated by **tabular Q-learning** from success/failure — no neural network trained anywhere, unlike Memento's Q-network or Retroformer's separate LoRA model. That makes it *more* scope-compatible than either (no gradient step at all), and it softens the "define from scratch" reading above to "there may be one adoptable *pattern* after all — a utility value per memory entry, updated by an external signal." It is **not** adoptable wholesale: its reward is automatic environment success (not a human/outcome signal — Casper's caveats apply), it has no forgetting/hard-delete, no human gate, and no versioning; and tabular Q-learning assumes dense, prompt, low-noise reward, which a legal pipeline with month-delayed case outcomes and multi-stage credit assignment does not provide. Queued Tier 2 (components A/C) in [`../papers/reading-queue.md`](../papers/reading-queue.md).
