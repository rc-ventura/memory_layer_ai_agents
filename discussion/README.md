...
| [`checkpoint-2026-08-21-infra-reflections.md`](checkpoint-2026-08-21-infra-reflections.md) | Reflections on the 21/08 infra meeting: sharper cross-trial/cross-agent confirmation, empirical (not just literature) confirmation of the forgetting gap inside the platform itself, a GraphRAG→LangGraph correction, the [API-INTERNA]/Kafka mechanism for systemic-mode signal capture, and three uncoordinated memory mechanisms already live in production. |
| [`checkpoint-2026-08-28-reflections.md`](checkpoint-2026-08-28-reflections.md) | Reflections on the 28/08 tutor checkpoint (Rafael presented the whole architecture hypothesis): tutor-side positive read on components C and D, the agent-type-dependent 13-month outcome-signal problem, cross-agent *reusable* memory back on the table (narrows a scope boundary), a human-initiated curator trigger, and a third primary-source reinforcement of harness-not-retraining. |
| [`reading-sprint-2026-08-24-queue-papers.md`](reading-sprint-2026-08-24-queue-papers.md) | Full-text findings from 9 parallel sub-agents reading the reading queue's architecture-relevant papers (SAGE, Memento, Mem-α, ExpeL, MemoryBank, Generative Agents, SCM, Retroformer, Casper et al.) — agent-read, not Rafael-read. Sharpens the controller-spectrum hypothesis and gives the central cross-trial × forgetting finding its first mechanism-level (not just survey-table) confirmation. |
| [`knowledge-as-infra-architecture-hypothesis.md`](knowledge-as-infra-architecture-hypothesis.md) | Working (not locked) architecture for the memory mechanism — six components (Memory Store, Signal Capture, Update Engine, Commit Gate, Forgetting, MCP Integration), worked through with a legal-flow example, built directly on the reading sprint's findings. Graduates to Sub 2.2 only after Sub 1.6/1.7 POC testing. |
| [`component-a-tutor-meeting-prep.md`](component-a-tutor-meeting-prep.md) | Briefing (PT) that distils component A (Memory Store) for a tutor meeting — what's settled in the design vs. the open gaps, and the works that decide each choice differently. |
| [`promotion-policy-log-to-ltm.md`](promotion-policy-log-to-ltm.md) | Design note (PT) consolidating the *promotion policy* — raw episodic log → embedded/indexed retrievable LTM — plus the three-tier memory model (raw log / retrievable episodic LTM / strategy layer) and the four write triggers that feed them, so promotion is situated among the other writes. The decided skeleton (`R = e^(−τ/S)`, DMF-lite `S`, `θ1` cut) plus the nine policy components still open (write-time `S` signals, use reinforcement, decay behaviour, failure-signal routing, capacity cap, dedup, evaluation cadence, promotion-`S`-vs-write-`S`, gate ownership). |
